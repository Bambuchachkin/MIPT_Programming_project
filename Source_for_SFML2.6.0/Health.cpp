...
void Health::update(float SCALE) {
    ...
    health_building[0].setPosition({bounds.getSize().x/4 + sprite->getPosition().x, sprite->getPosition().y - bounds.getSize().y/1});
    // Центрируем текст
    sf::FloatRect textBounds = health_building[0].getLocalBounds();
    health_building[0].setOrigin({textBounds.getSize().x / 2, 0});
